...
navLabel: Ratings
navOrder: 70
description: The Elo system behind the leaderboard, what makes a match count, and why ratings are never adjusted by hand.
published: true
---

The leaderboard is an Elo ladder replayed from the match record. Nothing on it is
entered by hand, and nothing on it can be.

As with the [metric definitions](/methodology), the text below is published
verbatim from the code's own documentation, and the two are checked against each
other.

{/* generated:start docs/modules/ratings.md */}
