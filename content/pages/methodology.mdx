...
navOrder: 60
description: How every number on this site is defined, in the same words the code is documented in.
published: true
---

Every statistic here is derived from event data and recomputable from it. None is
uploaded, and none is edited by hand.

The definitions below are **published verbatim from the code's own
documentation**. They are not a summary written afterwards: the file that
documents the metric modules and the text on this page are checked against each
other, and the site does not build when they disagree. Changing a definition
means changing both in the same change.

{/* generated:start docs/modules/metrics.md */}
...
The rating system has a page of its own: [How ratings work](/ratings-explained).
